...
{#if build_results?.required}
    ...
    >
        ...
        <DialogContent>
                ...
                {#if !complete && !loading}
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="flex flex-row justify-start items-center gap-4 ml-auto">
                            ...
                            <Select bind:value={very_hard_drop_buff} label="Normal">
                                {#each constants.drop_buff_options as option}
                                    <Option value={option.value}>{option.text}</Option>
                                {/each}
                            </Select>
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex flex-col">
                            ...
                            {#if stamina.days > 0 || stamina.hours > 0 || stamina.minutes || 0}
                                <small>
                                    {#if stamina.days > 0}
                                        <span>{stamina.days.toLocaleString("en-US")} days</span>
                                    {/if}
                                    {#if stamina.hours > 0}
                                        <span>{stamina.hours} hours</span>
                                    {/if}
                                    {#if stamina.minutes > 0}
                                        <span>{stamina.minutes} minutes</span>
                                    {/if}
                                    <span>
            ...
            {:else}
                <div class="flex flex-col justify-center items-center">
                    <strong class="mb-2">No Required Items Found</strong>
